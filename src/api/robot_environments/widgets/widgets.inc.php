...
function get_widget_by_id($id) {
  ...
  $sql = sprintf("SELECT * FROM `widgets` WHERE `widgetid`='%d'", api::cleanse($id));
...
function get_widget_instance_by_widgetid_and_id($widgetid, $id) {
  ...
  if ($w = get_widget_by_id($widgetid)) {
    // return what we find
    $result = array();
    $sql = sprintf("SELECT * FROM `%s` WHERE `id`='%d'", api::cleanse($w['table']),
    api::cleanse($id));
    return  mysqli_fetch_assoc(mysqli_query($db, $sql));
  }
...
function get_widget_by_script($script) {
  ...
  $sql = sprintf("SELECT * FROM `widgets` WHERE `script`='%s'", api::cleanse($script));
  return mysqli_fetch_assoc(mysqli_query($db, $sql));
}
...
function get_widget_by_table($table) {
  ...
  $sql = sprintf("SELECT * FROM `widgets` WHERE `table`='%s'", api::cleanse($table));
  return mysqli_fetch_assoc(mysqli_query($db, $sql));
}
    ...
    if ($table === $t) {
      ...
      foreach ($scripts as $s) {
        if ($script === $s) {
          ...
          api::cleanse($name), api::cleanse($table), api::cleanse($script));
          mysqli_query($db, $sql);
...
function create_widget_instance($entries) {
  ...
  if ($widget = get_widget_by_id($entries['widgetid'])) {
    ...
    $sql = sprintf("INSERT INTO `%s` (", api::cleanse($widget['table']));
    $insert = "";
    $values = "";
...
function update_widget_instance($fields) {
  ...
  // we can now run the update
  $sql = sprintf("UPDATE `%s` SET ".$sql." WHERE `id`='%d'", api::cleanse($widget['table'])
  , api::cleanse($fields['id']));
  mysqli_query($db, $sql);
...
function delete_widget_by_id($id) {
  ...
  if (get_widget_by_id($id)) {
    // delete it
    $sql = sprintf("DELETE FROM `widgets` WHERE `widgetid`='%d'", api::cleanse($id));
    mysqli_query($db, $sql);
    // no error
    return null;
  } else {
...
function delete_widget_instance_by_widgetid_and_id($widgetid, $id) {
  ...
  if ($widget = get_widget_by_id($widgetid)) {
    ...
    if (get_widget_instance_by_widgetid_and_id($widgetid, $id)) {
      // delete it
      $sql = sprintf("DELETE FROM `%s` WHERE `id`='%d'", api::cleanse($widget['table']),
      api::cleanse($id));
      mysqli_query($db, $sql);
      // no error
      return null;
