...
function get_environment_interface_pair_by_id($id) {
  ...
  $sql = sprintf("SELECT * FROM `environment_interface_pairs` WHERE `pairid`='%d'", api::cleanse($id));
...
function get_environment_interface_pair_by_envid_and_intid($envid, $intid) {
  ...
  api::cleanse($envid), api::cleanse($intid));
  return mysqli_fetch_assoc(mysqli_query($db, $sql));
}
...
function delete_environment_interface_pair_by_id($id) {
  ...
  if (get_environment_interface_pair_by_id($id)) {
    // delete it
    $sql = sprintf("DELETE FROM `environment_interface_pairs` WHERE `pairid`='%d'", api::cleanse($id));
    mysqli_query($db, $sql);
    // no error
    return null;
  } else {
