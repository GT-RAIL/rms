...
function get_interface_by_location($location) {
  ...
  $sql = sprintf("SELECT * FROM `interfaces` WHERE `location`='%s'", $db->real_escape_string($location));
  return mysqli_fetch_assoc(mysqli_query($db, $sql));
}
...
function update_interface($fields) {
  ...
  $sql .= sprintf(" `intid`='%d'", $db->real_escape_string($id_to_set));
  ...
  if(isset($fields['location'])) {
    ...
    $sql .= sprintf(", `location`='%s'", $db->real_escape_string($fields['location']));
  }
...
function delete_interface_by_id($id) {
  ...
  if(get_interface_by_id($id)) {
    // delete it
    $sql = sprintf("DELETE FROM `interfaces` WHERE `intid`='%d'", $db->real_escape_string($id));
    mysqli_query($db, $sql);
    // no error
    return null;
  } else {
